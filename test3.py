...
def process_frame(Y_current, ref_frame, block_size, search_range, mv_output_file):
...
def process_video(input_directory, height, width, block_size, search_range, mv_output_file_prefix):
    frame_counter = 0
    ...
    for file_path in y_files_full_path:
        ...
        reconstructed_frame, predicted_frame, residual_frame, avg_mae = process_frame(
            Y_current, Y_reference, block_size, search_range, mv_output_file
        )
...
process_video(input_directory, height, width, block_size, search_range, mv_output_file_prefix)
