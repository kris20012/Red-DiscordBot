    ...
    with open(residual_file, 'r') as f:
        for line in f:
            if line.startswith("Block Position"):
                ...
                # Parse floats first, then convert to integers
...
def process_decoded_video(ref_frame, num_frames, mv_file_prefix, residual_file_prefix, block_size):
    ...
    for frame_idx in range(num_frames):
        ...
        ref_frame = reconstructed_frame  # Use the reconstructed frame as the reference for the next frame
...
# Parameters
initial_reference_frame = np.full((height, width), 128, dtype=np.uint8)  # Or load actual reference frame
block_size = 8
num_frames = 5
